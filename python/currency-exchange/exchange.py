...
def exchange_money(budget: float, exchange_rate: float) -> float:
    ...
    """
    return float(format((budget / exchange_rate), '.2f'))
...
def get_change(budget: float, exchanging_value: float) -> float:
    ...
    """
    return float(format((budget - exchanging_value), '.2f'))
...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    """
    fee = spread / float(100)
    new_exchange_rate = exchange_rate * (1 + fee)
    exchanged_money = exchange_money(budget, new_exchange_rate)
    left = get_leftover_of_bills(exchanged_money, denomination)
    return int(exchanged_money - left)